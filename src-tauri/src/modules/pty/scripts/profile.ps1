# lithe-shell-integration (PowerShell)
...
if ($global:__LITHE_HOOKS_LOADED) { return }
$global:__LITHE_HOOKS_LOADED = $true
...
if (Test-Path Function:prompt) {
    Copy-Item Function:prompt Function:__lithe_user_prompt -Force -ErrorAction SilentlyContinue
}

function global:__lithe_urlencode {
    param([string]$s)
...
function global:__lithe_install_readline {
    if ($global:__lithe_readline_done) { return }
    if (-not (Test-Path Function:PSConsoleHostReadLine)) { return }
    $global:__lithe_readline_done = $true
    ...
    Copy-Item Function:PSConsoleHostReadLine Function:global:__lithe_user_readline -Force
    function global:PSConsoleHostReadLine {
        try {
            $line = __lithe_user_readline
        } catch {
            # Self-heal: restore the original reader so a broken wrapper can
            # never lock the shell out of input.
            Copy-Item Function:__lithe_user_readline Function:global:PSConsoleHostReadLine -Force
            return ''
        }
        try {
            if ($line -is [string] -and $line.Trim().Length -gt 0) {
                $global:__lithe_block_seen = $true
                $esc = [char]27
...
function global:prompt {
    ...
    if ($env:LITHE_BLOCKS) {
        ...
        $gap = if ($global:__lithe_block_seen) { "`n`n" } else { "`n" }
        return "$oscD$oscA$osc7$gap$oscB"
    }

    $original = if (Test-Path Function:__lithe_user_prompt) {
        try { & __lithe_user_prompt } catch { "PS $((Get-Location).Path)> " }
    } else {
