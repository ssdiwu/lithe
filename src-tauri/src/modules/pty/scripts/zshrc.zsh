...
# zsh, so we shadow $? into `_lithe_ret`.

{
  _lithe_user_zdotdir="${LITHE_USER_ZDOTDIR:-$HOME}"
  [ -f "$_lithe_user_zdotdir/.zshrc" ] && source "$_lithe_user_zdotdir/.zshrc"
...
if [[ -z "$__LITHE_HOOKS_LOADED" ]]; then
  __LITHE_HOOKS_LOADED=1
  ...
  _lithe_precmd() {
    local _lithe_ret=$?
    printf '\e]133;D;%s\e\\' "$_lithe_ret"
    printf '\e]7;file://%s%s\e\\' "${HOST}" "$(_lithe_urlencode "$PWD")"
  ...
  _lithe_preexec() {
    ...
    [[ -n "$LITHE_BLOCKS" ]] && _lithe_block_seen=1
    local cmd="${1//[[:cntrl:]]/ }"
    printf '\e]133;C;%s\e\\' "${cmd[1,256]}"
  }

  if (( $+functions[add-zsh-hook] )); then
    add-zsh-hook precmd _lithe_precmd
    add-zsh-hook preexec _lithe_preexec
  fi
