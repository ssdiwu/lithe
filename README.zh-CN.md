...
> [!NOTE]
> Lithe 目前是早期公开预览版。源码已经可以构建，但尚未发布官方安装包和
> 自动更新通道。目前已在 macOS 完成人工验证；Linux 与 Windows 仍是支持
> 目标，并由 CI 持续检查。
...
Lithe 目前没有公开的官方安装包。只应安装由你信任的源码和签名身份构建的
本地包。
...
Lithe 有意禁用自动更新。当前 release workflow 会在版本 tag 上创建 GitHub
草稿 release，并要求使用 Lithe 自己的签名凭据。详见
